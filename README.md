...
Script-based checks help flag likely untranslated lines and validate model suggestions. Japanese kanji shares its Unicode block with Chinese, so kanji without kana does not prove a line is untranslated. The review accepts short kanji-only names and model suggestions. For Chinese-to-Japanese dubs, it flags a longer line when its Han characters are carried over unchanged, including punctuation-only edits. This remains a heuristic: a changed Chinese sentence can escape it, and a long shared name may still be flagged.
